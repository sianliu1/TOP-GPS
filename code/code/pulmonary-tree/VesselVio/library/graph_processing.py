...
from VesselVio.library import feature_extraction as FeatExt, helpers, volume_processing as VolProc
...
def identify_edges(points, vertex_LUT, spaces):
    ...
    for i in range(points.shape[0]):
        ...
        for j in range(local.shape[0]):
            ...
            x, y, z = local[j, 0], local[j, 1], local[j, 2]

            # 鏄惧紡妫€鏌ユ瘡涓淮搴?            if x >= 0 and x < lut_shape[0]:
                if y >= 0 and y < lut_shape[1]:
                    if z >= 0 and z < lut_shape[2]:
                        target_index = vertex_LUT[x, y, z]
                        if target_index > 0:
