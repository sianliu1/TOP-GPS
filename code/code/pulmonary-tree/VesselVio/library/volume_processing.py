...
from VesselVio.library import  radii_corrections as RadCor
...
def binarize_and_bound_3D(volume):
    ...
    #
    # if non_zero[0].size == 0:  # Handle empty volume case
    #     mins = np.array(volume.shape, dtype=np.int_)
    #     maxes = np.zeros(3, dtype=np.int_)
    # else:
    #     # Compute min/max coordinates for bounding box
    #     mins = np.array([np.min(non_zero[i]) for i in range(3)], dtype=np.int_)
    #     maxes = np.array([np.max(non_zero[i]) for i in range(3)], dtype=np.int_)
    #
    #     # Binarize non-zero values in-place (avoiding full-array copy)
    #     volume[non_zero] = 1
    #
    # mins = np.maximum(mins-1, 0)
    # maxes = np.minimum(maxes+1, np.array(volume.shape) - 1)
    #
...
def skeletonize(volume, verbose=False):
    ...
    #     skeleton = skeletonize(skeleton, verbose=verbose)
