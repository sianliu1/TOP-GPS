...
from nnunet.inference.predict import predict_from_folder,predict_itk
...
def predict_itk_data_path(data_path,model_path=None):
    ...
    out_data = predict_itk(data_itk, lowres_segmentations, tta, mixed_precision, overwrite_all_in_gpu)
    out_itk = sitk.GetImageFromArray(out_data)
    out_itk.CopyInformation(data_itk)
    sitk.WriteImage(out_itk, 'E:/test.nii.gz')


    # return

def predict_itk_data(data_itk,model_name=None):

    lowres_segmentations = None
    overwrite_all_in_gpu = None
    tta = False
    mixed_precision = True
    print("using model stored in ", model_name)

    if model_name == "lung":
        tmp_np = sitk.GetArrayFromImage(data_itk)
        tmp_np = tmp_np[:,::-1,::-1]
        tmp_itk = sitk.GetImageFromArray(tmp_np)
        tmp_itk.CopyInformation(data_itk)

        out_data = predict_itk(tmp_itk,model_name, lowres_segmentations, tta, mixed_precision, overwrite_all_in_gpu)

        out_itk = sitk.GetImageFromArray(out_data[:,::-1,::-1])
        out_itk.CopyInformation(data_itk)

    else:
        out_data = predict_itk(data_itk, model_name, lowres_segmentations, tta, mixed_precision, overwrite_all_in_gpu)
        out_itk = GetImageFromArray(out_data)
        out_itk.CopyInformation(data_itk)
...
# def predict_npy(input_data:np.array,data_type):



if __name__ == "__main__":
    # main()
    predict_itk_data('D:\\nnUNet-master\\chest0438_0000.nii.gz')
