...
from nnunet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti,return_segmentation_from_softmax
...
from nnunet.training.model_restore import load_model_and_checkpoint_files,load_model_from_path,load_model_from_path2
...
def preprocess_itk_to_queue(preprocess_fn, q, data_itk, segs_from_prev_stage, classes,
                             transpose_forward):
    # suppress output
    # sys.stdout = open(os.devnull, 'w')
    ...
    d, _, dct = preprocess_fn(data_itk)
    # print(output_file, dct)
...
def preprocess_itk(trainer, data_itk, segs_from_prev_stage=None):
    if segs_from_prev_stage is None:
    ...
    pr = Process(target=preprocess_itk_to_queue, args=(trainer.preprocess_data_itk, q,
                                                        data_itk,
                                                        segs_from_prev_stage[0],
                                                        classes, trainer.plans['transpose_forward']))
    pr.start()
    processes.append(pr)
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        #                                   ))
        results.append(pool.starmap_async(return_segmentation_from_softmax,
                                          ((softmax_mean, dct, interpolation_order, region_class_order,
                                            None, None,
                                            npz_file, None, force_separate_z, interpolation_order_z),)
                                          ))
...
def predict_itk_case(data_itk,model_name, segs_from_prev_stage=None, do_tta=True, mixed_precision=True,
                  all_in_gpu=False, step_size=0.5,segmentation_export_kwargs: dict = None):
    """
    :param segmentation_export_kwargs:
    :param model: folder where the model is saved, must contain fold_x subfolders
    :param list_of_lists: [[case0_0000.nii.gz, case0_0001.nii.gz], [case1_0000.nii.gz, case1_0001.nii.gz], ...]
    ...
    model_path = "models"
    trainer,params = load_model_from_path2(model_path,model_name, mixed_precision=mixed_precision)
    ...
    print("starting preprocessing generator")
    preprocessing = preprocess_itk(trainer, data_itk,segs_from_prev_stage)
    print("starting prediction...")
    all_output_files = []
    data = []
    for process_data in preprocessing:
        output_filename, (d, dct) = process_data
        all_output_files.append(all_output_files)
        if isinstance(d, str):
            data = np.load(d)
            os.remove(d)
    ...
    #                                     npz_file, None, force_separate_z, interpolation_order_z),)
    #                                   ))
    result_image = return_segmentation_from_softmax(data[0], dct, interpolation_order, region_class_order)
    return result_image
...
def predict_itk( data_itk,model_name, lowres_segmentations: Union[str, None],tta: bool, mixed_precision: bool = True,overwrite_all_in_gpu: bool = None,
                step_size: float = 0.5,segmentation_export_kwargs: dict = None):
    ...
    return predict_itk_case(data_itk,model_name,lowres_segmentations, tta,
                         mixed_precision=mixed_precision, all_in_gpu=all_in_gpu,
                         step_size=step_size,
                         segmentation_export_kwargs=segmentation_export_kwargs)
