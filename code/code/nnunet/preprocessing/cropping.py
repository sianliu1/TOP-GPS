...
def load_data_from_itk(data_itk, seg_data_itk=None):
    ...
    properties["seg_file"] = seg_data_itk
    ...
    if seg_data_itk is not None:
        seg_itk = seg_data_itk
        seg_npy = sitk.GetArrayFromImage(seg_itk)[None].astype(np.float32)
    else:
        seg_npy = None
    return data_npy.astype(np.float32), seg_npy, properties
...
class ImageCropper(object):
    ...
    @staticmethod
    def crop_from_itk_imgs(data_itk, seg_itk=None):
        data, seg, properties = load_data_from_itk(data_itk, seg_itk)
        return ImageCropper.crop(data, properties, seg)
