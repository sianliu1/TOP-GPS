...
def load_model_from_path(model_path,model_name, mixed_precision=None, checkpoint_name="model_best"):

    trainer = restore_model(os.path.join(model_path,"%s.model.pkl" % model_name), fp16=mixed_precision)
    # trainer.output_folder = folder
    # trainer.output_folder_base = folder
    trainer.update_fold(0)
    trainer.initialize(False)
    all_best_model_files = [join(model_path, "%s.model" % model_name)]
    print("using the following model files: ", all_best_model_files)
    all_params = [torch.load(i, map_location=torch.device('cpu')) for i in all_best_model_files]
    return trainer, all_params

def load_model_from_path2(model_path,model_name, mixed_precision=None):

    trainer = restore_model(os.path.join(model_path,"%s.pkl" % model_name), fp16=mixed_precision)
    # trainer.output_folder = folder
    # trainer.output_folder_base = folder
    trainer.update_fold(0)
    trainer.initialize(False)
